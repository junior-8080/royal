  ...
  return (
    <section className={className}>
      <div className="w-full md:w-[80%] ml-auto ">
        <p className="text-base uppercase text-primary-default">{data.tag}</p>
      ...
      <div className="w-full md:w-[80%] mr-auto">
        <div className="flex flex-wrap items-center justify-between my-4">
          <div>
            {data.discount_price && (
              <span className="text-2xl line-through mr-4 text-[#aaaaaa]">
                {data.currency}
                {data.price}
              </span>
            )}
            <span className="text-2xl">
              {data.currency} {data.discount_price || data.price}
            </span>
          </div>
        ...
        <p className="text-xl my-4 text-[#777777]">{data.subtitle}</p>
        <div className="w-[50px] py-[2px] bg-orange-500 my-4"></div>
        {data.details.map((detail) => (
          <p key={detail} className="text-[#777777]">
            {detail}
          </p>
        ))}
      </div>
